...
# 同じ対話を Web サービスとして提供する（要 GEMINI_API_KEY）。cmd/server は API 専用
GEMINI_API_KEY=... go run ./cmd/server -addr :8080
# 別ターミナルでフロントエンドを開発モードで動かす場合（:5173 → :8080 に proxy）
cd web && npm install && npm run dev

# 本番: フロントエンドは S3 / R2 + CDN 等にビルド成果物をアップロードして配信し、
# cmd/server 側でそのオリジンを CORS 許可する（詳細は web/README.md）
...
| `cmd/server` | `internal/webapi` を HTTP サーバーとして起動する API 専用コマンド（`-static-dir` 指定時のみ静的配信も行う簡易モード） |
| `web` | フロントエンド（React + TypeScript + Vite）。チャット + 選択 UI。本番はオブジェクトストレージ + CDN から配信する想定 |
...
| フロントエンド | React + TypeScript + Vite（ビルド時のみ Node.js が要る。実行時は静的ファイル）。オブジェクトストレージ + CDN から配信 | [ADR-0016](docs/adr/0016-react-frontend-with-no-mvp-auth.md) / [ADR-0017](docs/adr/0017-frontend-served-from-object-storage-cdn.md) |
