...
* [x] **IR スキーマの確定**（M0 で v1 として確定。`internal/ir`）
  * `schemaVersion` フィールドで版を持つ。現在の値は `"1"`。読み込み時に空なら現行版として扱い、
    未対応の版はバリデーションで弾く
  * `Assumptions` は稼働時間・稼働日数・月間リクエスト数・為替レート・割引率の 5 項目に加えて、
    任意の前提を入れる `extra`（`map[string]float64`）を持つ
  * [ ] 残: 成長率・月次推移の期間を `Assumptions` の定型フィールドにするか（`extra` で足りるか）
  * [ ] 残: 保存済み IR の移行方針（版が上がったときに何をするか）
